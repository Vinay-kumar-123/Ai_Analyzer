...
export class YoutubeTranscriptProvider extends BaseTranscriptProvider {
  ...
  async fetch(videoId, { langConfig, attempt = 1, timeoutMs = TRANSCRIPT_FETCH_TIMEOUT_MS } = {}) {
    ...
    // ── PRODUCTION DIAGNOSTICS FOR THIS REQUEST ──
    let responseStatus = error?.response?.status ?? error?.status ?? null;
    let responseBodySnippet = error?.response?.data ?? error?.cause ?? null;
    let availableLanguages = [];
    let isAutoGenerated = false;
    let isManual = false;
    let isTranslatable = false;
    let captionsDisabled = true;
    let captionsListEmpty = true;
    ...
    try {
      ...
      if (responseStatus === null) responseStatus = rawRes.status;
      const html = await rawRes.text();
      if (!responseBodySnippet) responseBodySnippet = html ? html.substring(0, 300).replace(/\s+/g, " ") : null;
      ...
      if (playerResponseMatch) {
        try {
          ...
          if (captionTracks && Array.isArray(captionTracks)) {
            ...
            captionsListEmpty = captionTracks.length === 0;
            availableLanguages = captionTracks.map((t) => t.languageCode || t.name?.simpleText || "unknown");
            isAutoGenerated = captionTracks.some((t) => t.kind === "asr" || t.vssId?.startsWith("a."));
            isManual = captionTracks.some((t) => !t.kind && !t.vssId?.startsWith("a."));
            isTranslatable = captionTracks.some((t) => t.isTranslatable === true);
          } else if (playerResponse?.captions) {
            captionsDisabled = false;
            captionsListEmpty = true;
          }
        } catch (_) {}
      } else if (html.includes("consent.youtube.com")) {
        responseBodySnippet = `[CONSENT/BOT REDIRECT] ${responseBodySnippet}`;
      }
    } catch (diagErr) {
      if (responseStatus === null) responseStatus = `Diagnostic fetch error: ${diagErr.message}`;
    }

    const stackFirst10 = error?.stack
      ? error.stack.split("\n").slice(0, 10).join("\n")
      : "N/A (No error thrown)";

    const isDisabledErr = error?.name === "YoutubeTranscriptDisabledError" || (typeof YoutubeTranscriptDisabledError !== "undefined" && error instanceof YoutubeTranscriptDisabledError);
    const isNotAvailErr = error?.name === "YoutubeTranscriptNotAvailableError" || (typeof YoutubeTranscriptNotAvailableError !== "undefined" && error instanceof YoutubeTranscriptNotAvailableError);
    const isVidUnavailErr = error?.name === "YoutubeTranscriptVideoUnavailableError" || (typeof YoutubeTranscriptVideoUnavailableError !== "undefined" && error instanceof YoutubeTranscriptVideoUnavailableError);
    const isTooManyReqErr = error?.name === "YoutubeTranscriptTooManyRequestError" || (typeof YoutubeTranscriptTooManyRequestError !== "undefined" && error instanceof YoutubeTranscriptTooManyRequestError);

    const instanceofSummary = error
      ? `YoutubeTranscriptDisabledError: ${isDisabledErr}, YoutubeTranscriptNotAvailableError: ${isNotAvailErr}, YoutubeTranscriptVideoUnavailableError: ${isVidUnavailErr}, YoutubeTranscriptTooManyRequestError: ${isTooManyReqErr}, Error: ${error instanceof Error}`
      : "N/A (No error)";

    console.log(`
-------------------------------------------------
YOUTUBE-TRANSCRIPT PRODUCTION DIAGNOSTIC
-------------------------------------------------
VIDEO_ID: ${videoId}
Original exception class: ${error ? (error.constructor ? error.constructor.name : typeof error) : "NONE (SUCCESS)"}
constructor.name: ${error?.constructor?.name || "N/A"}
instanceof checks: ${instanceofSummary}
error.name: ${error?.name || "N/A"}
error.message: ${error?.message || "N/A"}
error.stack (first 10 lines):
${stackFirst10}
response status (if available): ${responseStatus ?? "N/A"}
response body (if available): ${responseBodySnippet ?? "N/A"}
all available transcript languages: ${JSON.stringify(availableLanguages)}
whether captions are auto-generated: ${isAutoGenerated}
whether captions are manual: ${isManual}
whether captions are translatable: ${isTranslatable}
whether captions are disabled: ${captionsDisabled}
whether captions list was empty: ${captionsListEmpty}
what URL was requested: ${targetUrl}
which language candidate was used: ${langLabel}
-------------------------------------------------
RAW EXCEPTION PRINT:
`, error);

    if (error) {
      // Re-throw raw exception directly without modifying or classifying it
      throw error;
    }
